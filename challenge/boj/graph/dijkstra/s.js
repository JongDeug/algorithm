...
// 틀린 문제
// 핵심
// 1. 이 문제에는 가중치가 없다고 보면 됨. 그냥 최단 거리가 K 인 것을 고르면 되기 때문에 굳이 우선 순위 큐를 사용해서 문제를 풀 필요가 없음

// [문제 세분화] : BFS, queue 를 사용
function parseInput(input) {
    ...
    return { N, K, X, graph, distances };
}

function solution() {
    // I. 입력 받기
    const { K, X, graph, distances } = parseInput(input);
    const ans = [];
    ...
    const BFS = (start) => {
        ...
        while (queue.length) {
            ...
            // I. 최적화를 위해 K 이하인 것만, K 만 구하면 되는데 K 이상을 구할 필요가 없음
            // I. 만약 이게 없다면 최악의 경우 O(N^2) 이 나올 수 있음
            if (distances[node] <= K) {
    ...
    distances.forEach((v, idx) => {
        if (v === K) ans.push(idx);
    });

    return ans.length ? ans.sort((a, b) => a - b).join("\n") : -1;
}

console.log(solution());

// // [문제 세분화] : 다익스트라 우선순위 큐 사용
// class Node {
